...
它是性能最高、最常用的智能指针，作用：同一时间只能有一个 unique_ptr 指向该资源。

2.***std::shared_ptr —— 共享式所有权***

需要多个指针共同指向同一个对象时，使用它。（所有指向同一个对象的 std::shared_ptr，其内部的指针指向同一块内存区域）

作用：通过**引用计数（Reference Count）**管理资源。

用法：可以随意拷贝。每多一个指针指向它，计数 +1；每销毁一个指针，计数 -1。当计数归零时，删除资源。

原理：包含两个指针，一个指向对象，一个指向控制块（存放计数器p1.use_count()，打印出所有拷贝对象的数目）

3.std::weak_ptr —— 观察者

它是 shared_ptr 的辅助工具，不控制对象的生命周期。

作用：指向 shared_ptr 管理的对象，但不增加引用计数。主要用于解决循环引用问题

这里就要介绍一下他们之间的作用了：首先看以下代码
```
#include <iostream>
#include <memory>
...
struct B {
    ❤️std::weak_ptr<A> ptrA; // 使用 weak_ptr 破解循环
    ~B() { std::cout << "B 销毁\n"; }
};
...
```
***以上的关系中，A,B是相互拥有的，注意❤️处代码行如果使用shared_ptr<A>的话***
```
auto a = std::make_shared<A>();

此时有一个 shared_ptr 指向对象 A。对象 A 内部的 ptrB 目前还是空的。

引用计数：A (1), B (0)。

auto b = std::make_shared<B>();

此时有一个 shared_ptr 指向对象 B。对象 B 内部的 ptrA 也是空的。

引用计数：A (1), B (1)。

a->ptrB = b;  <-- 关键点 1

这一步让对象 A 内部的 shared_ptr 指向了对象 B。

因为是 shared_ptr 赋值，B 的引用计数加 1。

引用计数：A (1), B (2)。这时候，对象 A “拥有”了对象 B。

b->ptrA = a; <-- 关键点 2（如果这里用的是 shared_ptr）

如果 B 里的 ptrA 也是 shared_ptr：

这一步让对象 B 内部的 shared_ptr 指向了对象 A。

A 的引用计数加 1。

引用计数：A (2), B (2)。这时候，对象 B 也“拥有”了对象 A
```
***那么，当我试图销毁内存的时候🚨***
```
当 main 函数结束时：

栈上的局部变量 a 销毁，A 的计数减 1（变成 1）。

栈上的局部变量 b 销毁，B 的计数减 1（变成 1）。

现在尴尬的事情发生了🏴‍☠️：

对象 A 想销毁，必须等它的成员 ptrB 销毁（即 B 的计数归零）。

对象 B 想销毁，必须等它的成员 ptrA 销毁（即 A 的计数归零）。

A 等 B，B 等 A。结果就是谁的析构函数都不会被执行，内存永久泄漏
```
现在就是`std::weak_ptr`出场的时候了：
这时我们执行 引用b->ptrA = a; 时：

虽然 ptrA 指向了 A，但因为它是一个“弱引用”，它不会增加 A 的引用计数，所以A试图删除的时候的时候不需要看B的脸色，因为B相当于没有真正引用到A上面去。

In conclusion:🧑‍🏫
如何识别循环引用

在复杂的项目中（比如你提到的并行架构或推理引擎设计），识别循环引用的方法是画所有权图：

如果图里出现了一个闭环（A -> B -> A），且路径上全部是 shared_ptr，那就是内存泄漏的定时炸弹。

***解决办法永远是：把环里的某一条边改成 weak_ptr。通常遵循“上级管下级用 shared，下级看上级用 weak”的原则***


# C++ 内存管理特性
主要分为五个区域：***栈，堆，全局静态区，常量存储区，代码区***，接下来🍓将详细介绍：

## 栈（Stack)
由编译器自动分配释放的***一块连续内存区域***，主要存放局部变量、函数参数、返回地址。遵循“LIFO”原则。
分配内存只需要移动一下指针，速度较快。随着函数调用的结束而自动销毁，不用手动销毁管理。

## 堆（Heap)
由程序员手动分配和释放的***很多片离散的地址块（通过指针连接）***，主要是通过调用函数malloc()和free()申请和手动释放。
动态分配，速度相对较慢，靠指针链接因此寻址较慢。

## 全局/静态存储区 (Global/Static Storage)
存储全局变量和静态变量（static）。程序启动时分配，程序结束时释放。全局区中还包含常量区，字符串常量和其他常量也存储在这里。

## 常量区
存储常量字符串等不允许修改的数据。是全局区的一部分。

## 代码区
代码区存储的是程序编译后生成的机器指令（Binary Code），包含函数体，常量字符串和之前提到的虚函数表等。运行方式参考CSAPP的CPU指令
存取方式。


***🍓‘s 🫴🥝TIPS：malloc是怎么申请内存空间的？***

***采取二级管理流程：***

第一步：查找用户态内存池（The Arena）

malloc 内部维护着一个内存池。

如果池子里有现成的、大小合适的空闲内存块，它会直接打上“已占用”标记并返回。

这避免了频繁陷入内核态，速度极快。

第二步：向内核申请（Brk 或 Mmap）

如果内存池不够用了，malloc 会调用系统调用向内核索要（OS介入）。策略取决于申请的大小

:shipit:***reminds you!!!malloc 返回的是虚拟地址空间中的起始地址**。① 返回值的类型：void*；② 返回的是虚拟地址，而非物理地址。

在这里🥑要提醒一下，在申请内存空间的时候会有两种错误：缺页异常(Page Fault)和段错误(Segment Fault)

由于malloc返回的是一个虚拟物理地址，只有你需要往里面写数据的时候MMU才会一顿查发现：哎嘿这个地址不存在(valid bit=0)。那让我把控制权交给操作系统的缺页中断处理函数 (Page Fault Handler)看看怎么处理：

第一步：这个虚拟地址合法吗？

内核会检查进程的内存区域表（VMA, Virtual Memory Areas）。

如果不合法：比如你访问的是 0x0（空指针），或者超出了堆、栈的边界。

结果：判定为 段错误 (Segmentation Fault)，直接杀掉进程。

第二步：如果合法，权限对吗？
如果地址合法（在你的领地内），内核接着看你想干什么。

违规操作：比如这个页是只读的（代码区），你却想写它。

结果：判定为 段错误，杀掉进程。

第三步：如果地址合法且权限正确，为什么报错？
内核发现地址没问题，权限也没问题，只是页表里显示该页不在内存中。

原因：物理内存还没分，或者被置换到磁盘了。

结果：判定为 缺页异常 (Page Fault)，这个时候OS介入从RAM或者磁盘调用物理空间，完成后让程序继续运行。
...
## 动态链接库是怎么被装载到内存的？使用***mmap***
它在进程的虚拟内存中划出一块区域，声明这块区域就是某个文件的“镜像”。此时并没有真正的物理拷贝，只有当你真正去读这段内存时，操作系统才会通过缺页中断把数据从磁盘加载到物理内存。
